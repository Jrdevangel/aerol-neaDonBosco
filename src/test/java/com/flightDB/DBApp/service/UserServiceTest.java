...
class UserServiceTest {
    ...
    void getUserById() {
        ...
        User user = new User(1L, "Angel", "1234", "angel@gmail.com", eRole, null);
        when(iUserRepository.findById(1L)).thenReturn(Optional.of(user));
        User result = userService.getUserById(1L);
        assertNotNull(result);
        assertEquals(1L, result.getId());
        assertEquals("Angel", result.getUsername());
    }

    @Test
    void updateRole() {
        ERole eRole = ERole.ADMIN;
        ERole Angel = ERole.USER;
        User user = new User(1L, "Maksym", "1234", "maksym@gmail.com", Angel, null);
        when(iUserRepository.save(user)).thenReturn(user);
        when(iUserRepository.findById(1L)).thenReturn(Optional.of(user));
        User result = userService.updateRole(eRole, 1L);
        assertEquals(1L, result.getId());
